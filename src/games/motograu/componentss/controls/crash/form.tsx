...
import Tabs from '@/games/motograu/componentss/tabs'
...
export default function CrashForm({
  ...
  const disabled = transaction.status != TransactionStatus.UNREGISTERED

  return (
    <div className="bg-black border border-gray-600 bg-opacity-20 border-opacity-20 crash-form w-full h-45  flex rounded-md p-3 relative">
      <If condition={toggleSecond && !secondEnabled}>
        <button
          onClick={toggleSecond}
          className={`btn border-none bg-[#ffffff] bg-opacity-10 btn-xs btn-circle absolute px-1 mt-1 right-3 sm:hidden`}
        >
      ...
      >
        ...
        <section className="flex flex-col mx-auto gap-3">
          ...
          <div className="w-full font-lexend">
            ...
            >
              <button
                className={`btn normal-case rounded-md text-lg font-normal text-white border-none ${getBackgroundColor(
                  color
                )} bg-[#ff7700] hover:bg-[#d26200] flex flex-col px-0 h-full w-full`}
              >
                <span className="">
                  Iniciar o jogo
                  {transaction.mode == TransactionMode.COMMON ||
                    <span className="text-base">
                      &nbsp;({transaction.roundCount || transaction.roundCount + 1})
                    </span>
                  }
                </span>
            ...
            >
              <button
                className={`btn normal-case rounded-md text-lg font-normal text-white border-none px-0 h-full w-full bg-red-700 hover:bg-red-800`}
            ...
            >
              <div className="flex flex-col w-full h-full">
                <button
                  className={`btn normal-case rounded-md text-lg font-normal text-white border-none bg-red-700 hover:bg-red-800 px-0 h-full w-full`}
                ...
                >
                  ...
                  <If condition={!transaction.autoStarted}>
                    <span className="">Cancelar x</span>
                  </If> */}
                </button>
              </div>
            </If>
            ...
            >
              <button
                className={`btn normal-case rounded-md text-lg font-normal text-white border-none bg-[#ff7700] hover:bg-[#d26200] px-0 h-full w-full`}
